  ...
  return (
    <div className="space-y-6 sticky top-[72px]">
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-xs">
        <h3 className="text-xs font-black uppercase tracking-wider text-[var(--orange)] mb-3">⚡ Weekly Progress</h3>
        <div className="space-y-3">
          ...
          <p className="text-[10px] text-[var(--text-muted)] font-medium">Solve {weeklyProgress.target} problems weekly to accelerate XP multiplier.</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-xs">
        <div className="flex justify-between items-center mb-3">
          <h3 className="text-xs font-black uppercase tracking-wider text-[var(--orange)]">📅 Contribution Grid</h3>
          <span className="text-[9px] text-[var(--text-muted)] font-bold uppercase">Past 15 Weeks</span>
        </div>
        
        {/* Heatmap Grid SVG Layout */}
        <div className="flex justify-center p-2 rounded-xl bg-[var(--surface)] border border-[var(--border-light)] overflow-x-auto">
        ...
        <div className="flex justify-between items-center text-[9px] text-[var(--text-muted)] mt-2.5 font-bold uppercase">
          <span>Less</span>
          <div className="flex gap-0.5 items-center">
            <span className="w-2 h-2 rounded-[1px] bg-[var(--border)] opacity-45" />
            <span className="w-2 h-2 rounded-[1px] bg-[var(--orange)]" />
          </div>
          <span>More</span>
        </div>
      </div>
      ...
      {/* 🏆 Achievements Card */}
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-xs">
        <h3 className="text-xs font-black uppercase tracking-wider text-[var(--orange)] mb-3">🏆 Unlock Achievements</h3>
          ...
          {achievements.map((badge) => (
            <div 
              ...
              className={`flex items-center gap-3 p-2.5 rounded-xl border transition-all ${
                badge.unlocked 
                  ? 'bg-[var(--surface)] border-[var(--border)] opacity-100' 
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex justify-between items-center">
                  <p className="text-xs font-extrabold text-[var(--text)] leading-none truncate">{badge.name}</p>
                  {badge.unlocked ? (
                ...
                <p className="text-[10px] text-[var(--text-muted)] mt-1 truncate">{badge.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-5 shadow-xs">
        <div className="flex justify-between items-center mb-3">
          <h3 className="text-xs font-black uppercase tracking-wider text-[var(--orange)]">🥇 Top Members</h3>
          <Link to="/challenges/leaderboard" className="text-[10px] font-bold text-[var(--orange)] hover:underline uppercase tracking-wide">All Standings</Link>
        </div>
        ...
        <div className="divide-y divide-[var(--border-light)]">
          {topMembers.map((member, index) => {
            ...
            return (
              <div key={member.id} className={`flex items-center justify-between py-2.5 ${isSelf ? 'bg-[var(--orange)]/5 -mx-5 px-5 rounded-lg' : ''}`}>
                <div className="flex items-center gap-2.5 min-w-0">
                  <span className="text-xs font-extrabold w-4 text-[var(--text-muted)] text-center">
                  ...
                  />
                  <span className={`text-xs truncate font-bold ${isSelf ? 'text-[var(--orange)]' : 'text-[var(--text)]'}`}>{member.name}</span>
                </div>
